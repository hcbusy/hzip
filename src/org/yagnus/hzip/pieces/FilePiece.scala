...
class FilePieceMaker
    ...
    def toXml =
        <FilePieceSpec>
            {
                //println("inside toXml, the key set is "+mapping.keySet.size+", the iterator has size "+getAllKeys.size);
                for (fn ← getAllKeys)
                    yield <FILE>
    ...
    override def add(index : String, len : Long) {
        ...
        //println("add of file piece maker being called with "+index+","+len+", after add, the key size is "+getAllKeys.size);
    }
