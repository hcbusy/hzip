 ...
 */
class PieceCreator(fs : FileSystem, inFiles : Seq[String], inDirs : Seq[String], var outputDir : String, pieceMaker : FilePieceMaker)
    extends HadoopCompressionAlgorithm(fs) {

    private lazy val _l = new HZLogger(classOf[PieceCreator]);

    def this(fs : FileSystem, inFiles : Seq[String], inDirs : Seq[String], outputDir : String, algo : Algorithms.Algorithms, pieceMaker : FilePieceMaker) {
        this(fs, inFiles, inDirs, outputDir, pieceMaker);
    ...
    def createCompressionJob() : CompressionJob = {
        val jb = new JobConf();
        //        for(fn <- inFiles){
        //            println("Going to compress "+fn +" into "+outputDir);
        //        }
        ...
        val ret = new CompressionJob(jb) {
            override def postHadoop() = {
                ...
                //println("The pieceMaker looks like this:"+pieceMaker.toXml);
...
class PieceMapper extends CompressionMRBase
 ...
 *
 */
class PieceReducer(compressTo : OutputStream ⇒ OutputStream) extends CompressionMRBase
