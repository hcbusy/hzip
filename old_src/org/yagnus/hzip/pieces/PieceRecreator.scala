    ...
    def createCompressionJob() : CompressionJob =
        {
            ...
            jb.setStrings("mapred.input.dir", source);
            FileInputFormat.setInputPaths(jb, source + Common.OUTPUT_HZ_DATA_FILE);
            if (!outputDir.endsWith("/")) outputDir += "/";
            FileOutputFormat.setOutputPath(jb, outputDir+"temp");
...
class PieceRecreatorMapper(decompressor : (InputStream ⇒ InputStream))
    ...
    def map(key : LongArrayWritable, value : BytesWritable,
        ...
        val inputStream = new ByteArrayInputStream(value.get);
        ...
        while (decompressedStream.available > 0) {
            val len = decompressedStream.read(ab);
            if (len > 0) {
...
class PieceRecreatorReducer extends CompressionBase
